...
if( $_POST['upload'] == 'Y' ) 
	{
		if ( $_POST['copyit'] == 'yes' ) 
		{
		echo "<center><h1>the 'COPY FROM LOCATION' feature will be ready in the next version</h1> For now please upload a file !";
		}
		...
		if($_FILES['sponsor_file']['error'] === 0) 
		{
			...
			if ((substr($file_location,-3) != "txt") && (substr($file_location,-3) != "csv") && (substr($file_location,-3) != ".gz"))// nogo on zip
				{
				exit ("<div style='color:red;font-weight:900'>I am not clear on this file. Please upload files with .txt , .GZ or .csv extensions only !</div>");
				}			
		}
	}
...
Select a datafeed stored on your computer ! I can deal with .txt .csv or .gz files (I'll automatically unzip files too).<br>
	<?PHP 
	echo "--> your php.ini allows a maximum of ".ini_get('post_max_size')." for post-size and a max of ".ini_get('upload_max_filesize')." for upload<br>"; 
	
		if (!is_writable($uploads['path'])) 
			{
			echo "<div style='color:red;font-weight:900'>ERROR ! I can not save files in the directory: {$uploads['path']}<br>We need this directory to be writable in order to upload your CSV file !<br>
			Most likely you have directory permission issues because you installed WordPress manually or with non-recommended settings.<br>
			You can provide permissions to the upload directory as set out below, to get this plugin working, but this is just a band-aid.<br>
			You are likely going to have more problems with WordPress in general and other plugins.<hr>
			To get this plugin going again please provide 777 permission to directory: <br>{$uploads['path']}
			</div>";
			exit();
			}
		else 
			{
			echo "--> {$uploads['path']} checked & found OK for writing, please proceed:<br>";
			}
	?>
<br>
	...
	<form name="l66_upload_file" enctype="multipart/form-data" method="post">
		<input type="hidden" name="upload" value="Y">
		<input type="text" name="urlloc" value="enter a url to your datafeed">
		<input type="hidden" name="copyit" value="yes">
	<input type="submit" style="background-color:yellow" name="Submit" value="<?php _e('copy from location!')?>" >
	</form>
</div>
...
<div style="font-weight:900;color:blue">Currently selected separator :  -> <?PHP echo $del; ?> <- </div> 
Now Look above and tell me how this feed is separated:
<select name="preset">
...
<input type="submit" style="background-color:yellow" name="Submit" value="<?php _e('change separator !')?>" />	
</form>
<!-- end delimiter form -->			
			...
			for ($c=0; $c < $num; $c++)
				...
				$csv_columns[$c] = substr($csv_columns[$c],0,30);
						...
						if ($_POST['col'.$A] == $c) {$select = "selected";} else
							{
						$sku = array("sku", "SKU", "identifier", "product ID"); // 3 = sku
						if ($A == 3) {if (in_array($csv_columns[$c],$sku)) {$select = "selected";} }
						$title = array("title", "Title","name","NAME","Product_Name"); // 4 = title array
						if ($A == 4) {if (in_array($csv_columns[$c],$title)) {$select = "selected";} }
						$desc = array("desc", "Desc", "Description","description", "DESCRIPTION","Product_Desc"); // 5 = description
						...
						$lin = array("productURL", "product","BUYURL","url", "Product_Tracking_URL"); // 
						if ($A == 10) {if (in_array($csv_columns[$c],$lin)) {$select = "selected";} }
						$img = array("image", "imageURL","IMAGEURL","image","Product_Image_URL"); // 
						if ($A == 11) {if (in_array($csv_columns[$c],$img)) {$select = "selected";} }
						$pri = array("price", "Price","PRICE", "Product_Price"); // 
						if ($A == 12) {if (in_array($csv_columns[$c],$pri)) {$select = "selected";} }
						$pri = array("retailprice", "msrp","oldprice","RETAILPRICE","price_old", "Product_Sale_Price"); // 
						if ($A == 13) {if (in_array($csv_columns[$c],$pri)) {$select = "selected";} }
						$pri = array("currency", "CURRENCY","curr", "Product_Currency"); // 
