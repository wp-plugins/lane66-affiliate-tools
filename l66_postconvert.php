...
if (isset($_POST['post1'])) 
		...
		$vals=$_POST['incfeed'];
		if (empty($vals)) {echo'<h2><font color="red">you did not select any feeds that I can choose from ! Please try again.</font></h2>';echo "</div>";}
		else
		{
		...
		$content = "<div style='width:100%;overflow:hidden;text-align:center;'><A href='$row->link' title='$row->descr'><img src='$row->image' width='200' alt='lane66.com datafeed loader'></a><hr>$row->descr<hr><br>Price : <A href='$row->link' title='$row->title'>$row->currency $row->price</a></div>";
		
		if (get_option('l66_modus') == "freebie") 
			{
			$content.="<hr><div style='text-align:center;color:#BDBDBD;font-size:10px;'>Created with a free version of <a href='http://lane66.com' title='lane66.com builds professional affiliate plugins to help you make money with datafeeds. Most of their plugins are free. Visit them now ....'>lane66.com</a> affiliate tools PRO-series</div>";
			}
...
?>
	...
	<input type="text" name="tags" value="lane66.com,feed-monster.com" size="40" onFocus="clearText(this)"/> <hr>
	...
	<input type="radio" name="skipnopic" value="on" <?PHP if (get_option('l66_modus') == "freebie") {?> onClick="this.checked=false; alert('Sorry, this option is not available in the free version. Please consider upgrading to PREMIUM membership. You can do so on the bottom of this screen.')"<?PHP ;} ?>> yes 
	...
	<input type="radio" name="l66-status" value="pending" <?PHP if (get_option('l66_modus') == "freebie") {?> onClick="this.checked=false; alert('Sorry, this option is not available in the free version. Please consider upgrading to PREMIUM membership. You can do so on the bottom of this screen.')"<?PHP ;} ?>> pending review
	<input type="radio" name="l66-status" value="draft" <?PHP if (get_option('l66_modus') == "freebie") {?> onClick="this.checked=false; alert('Sorry, this option is not available in the free version. Please consider upgrading to PREMIUM membership. You can do so on the bottom of this screen.')"<?PHP ;} ?>> draft 
	<input type="radio" name="l66-status" value="private" <?PHP if (get_option('l66_modus') == "freebie") {?> onClick="this.checked=false; alert('Sorry, this option is not available in the free version. Please consider upgrading to PREMIUM membership. You can do so on the bottom of this screen.')"<?PHP ;} ?>> private 
	<hr>
	...
	<input type="radio" name="product" value="100" <?PHP if (get_option('l66_modus') == "freebie") {?> onClick="this.checked=false; alert('Sorry, this option is not available in the free version. Please consider upgrading to PREMIUM membership. You can do so on the bottom of this screen.')" <?PHP ;} ?> > 100
	<input type="radio" name="product" value="999999" <?PHP if (get_option('l66_modus') == "freebie") {?> onClick="this.checked=false; alert('Sorry, this option is not available in the free version. Please consider upgrading to PREMIUM membership. You can do so on the bottom of this screen.')" <?PHP ;} ?> > all of them
	</div>
