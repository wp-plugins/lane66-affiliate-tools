...
echo "<h2>You are currently operating in <font color='blue'>".get_option('l66_mode')."</font> mode !</h2>";
...
$maxmerch = $wpdb->get_var("SELECT COUNT(DISTINCT merchantid) FROM $table_name"); // number of distinct feeds
...
foreach ($view as $row) 
		{
	echo "<font color='blue'>".$row->feedname." (".$row->network.")</font><br>" ;
		}

?>
<hr>
<p align='center'><center>
Click the button below to wipe the entire lane66.com database clean.<br><b>**** WARNING ****</b><br> there will be NO way back once you click this button. All your affiliate products will instantly be removed.<br>
<form name="kill_table" enctype="multipart/form-data" method="post">
<input type="hidden" name="killit" value="Y">
<input type="submit" name="Submit" value="<?php _e('Destroy and remove all products from the affiliate database ?')?>" />
</form>	
<?PHP
...
Below is a list of items in your upload directory (As also used by the "upload my own csv" tool). Select items some to delete them.<br>
						...
						?>
						<div style="width:450px;height:100px;padding:5px;overflow:auto;border:1px solid #ccc">
...
<h3>Our forum - post all lane66.com related posts here</h3>
<iframe src="http://lane66.com/forum" width="100%" height="2000"></iframe>	
</div>				